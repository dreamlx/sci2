...
RSpec.describe FeeDetailStatusService, type: :service do
  ...
  describe '#update_status' do
    ...
    context '当费用明细只关联沟通工单时' do
      let(:communication_work_order) do
        create(:communication_work_order, reimbursement: reimbursement, status: 'completed')
      end
    ...
    context '当有多个审核工单和沟通工单时' do
      let(:old_audit_work_order) do
        create(:audit_work_order, reimbursement: reimbursement, status: 'rejected', created_at: 1.day.ago)
      end
      let(:new_audit_work_order) do
        create(:audit_work_order, reimbursement: reimbursement, status: 'approved', created_at: 1.hour.ago)
      end
      let(:communication_work_order) do
        create(:communication_work_order, reimbursement: reimbursement, status: 'completed', created_at: 30.minutes.ago)
      end
  ...
  describe '#get_latest_work_order' do
    ...
    context '当有审核工单和沟通工单时' do
      ...
      let(:communication_work_order) do
        create(:communication_work_order, reimbursement: reimbursement, created_at: 30.minutes.ago)
      end
