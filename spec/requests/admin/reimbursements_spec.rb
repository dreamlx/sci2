...
RSpec.describe 'Admin::Reimbursements', type: :request do
  ...
  describe 'PUT /admin/reimbursements/:id' do
    it '更新报销单' do
      put admin_reimbursement_path(reimbursement),
          params: { reimbursement: { document_name: 'Updated Name', is_electronic: true } }
  ...
  describe 'POST /admin/reimbursements/import' do
    ...
    it '调用导入服务并重定向' do
      # 模拟导入服务成功
      service_double = instance_double(ReimbursementImportService,
                                       import: { success: true, created: 1, updated: 0, errors: 0 })
      allow(ReimbursementImportService).to receive(:new).with(instance_of(ActionDispatch::Http::UploadedFile),
