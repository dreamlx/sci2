...
RSpec.describe '表单字段一致性', type: :feature do
  ...
  describe '状态字段只读性' do
    ...
    let!(:communication_work_order) do
      create(:communication_work_order, reimbursement: reimbursement, status: 'pending')
    end
  ...
  describe '处理意见与状态关系', js: true do
    let!(:audit_work_order) do
      create(:audit_work_order, reimbursement: reimbursement, status: 'pending', problem_type: '发票问题')
    end
