...
RSpec.describe 'Full Workflow Integration', type: :model do
  ...
  let(:operation_histories_approval_csv_path) do
    Rails.root.join('spec/fixtures/files/test_operation_histories_approval.csv')
  end
  ...
  scenario 'simulating a full reimbursement processing workflow' do
    ...
    unless import_result[:success]
      ...
      if import_result[:unmatched_details].present?
        puts "Unmatched details: #{import_result[:unmatched_details].map do |d|
          "#{d[:document_number]} (#{d[:error]})"
        end.join(', ')}"
      end
    end
    ...
    # 添加沟通记录
    comm_wo1.add_communication_record(content: '已与张伟电话沟通，确认金额调整为3000元', communicator_role: '张经理',
    ...
    comm_wo_approve.add_communication_record(content: '已与陈明电话沟通，已收到参会人员名单', communicator_role: '张经理',
                                             communication_method: '电话')
    ...
    import_result = OperationHistoryImportService.new(operation_histories_approval_csv_path,
                                                      admin_user).import(spreadsheet)
    puts "Import result: #{import_result.inspect}"
    expect(OperationHistory.count).to eq(16) # 初始10条 + 新导入6条
    puts "审批操作历史导入完成，共 #{OperationHistory.count} 条。"

    (1..6).each do |i|
      reimbursement = if i <= 5
                        Reimbursement.find_by(invoice_number: "ER1422825#{i}")
                      else
                        Reimbursement.find_by(invoice_number: 'R2025%03d' % i)
                      end
  ...
  def create_test_csv_files
    ...
    CSV.open(reimbursements_csv_path, 'w') do |csv|
      ...
      (1..5).each do |i|
        csv << ["ER1422825#{i}", "报销单#{i}", "申请人#{i}", 'E%03d' % i, '科技有限公司', "部门#{i}", 'pending', '',
                '2025-04-%02d' % i, (100 + (i * 50)).round(2), '待审批']
      end
      # Remaining records use R2025006-R2025010 format
      (6..10).each do |i|
        csv << ['R2025%03d' % i, "报销单#{i}", "申请人#{i}", 'E%03d' % i, '科技有限公司', "部门#{i}", 'pending', '',
                '2025-04-%02d' % i, (100 + (i * 50)).round(2), '待审批']
      end
    ...
    CSV.open(express_receipts_csv_path, 'w') do |csv|
      csv << %w[单据编号 操作类型 操作日期 操作人 操作意见]
      # First 5 records use ER14228251-ER14228255 format
      (1..5).each do |i|
        csv << ["ER1422825#{i}", '收单', format('2025-04-%02d', i + 10), '张经理', "快递单号：SF100#{i}"]
      end
      # Remaining records use R2025006-R2025010 format
      (6..10).each do |i|
        csv << ['R2025%03d' % i, '收单', format('2025-04-%02d', i + 10), '张经理', '快递单号：SF%010d' % i]
      end
    end
    ...
    CSV.open(fee_details_csv_path, 'w') do |csv|
      csv << %w[报销单单号 费用类型 原始金额 原始币种 费用发生日期 弹性字段11]
    ...
    CSV.open(operation_histories_approval_csv_path, 'w') do |csv|
      ...
      # First 5 records use ER14228251-ER14228255 format
      (1..5).each do |i|
        csv << ["ER1422825#{i}", '审批', format('2025-04-%02d', i + 20), '李总', '审批通过']
      end
      # Plus R2025006
      csv << %w[R2025006 审批 2025-04-26 李总 审批通过]
    end
