...
RSpec.describe AuditWorkOrder, type: :model do
  ...
  subject { build(:audit_work_order, reimbursement: reimbursement, creator: admin_user) }
  ...
  describe 'validations' do
    ...
    # type is automatically set by STI, no need to validate
    ...
    it 'validates presence of problem_type when rejected' do
      work_order = build(:audit_work_order, reimbursement: reimbursement, status: 'rejected', problem_type: nil,
                                            audit_result: 'rejected')
      expect(work_order).not_to be_valid
      expect(work_order.errors[:problem_type]).to include('不能为空')

      work_order = build(:audit_work_order, reimbursement: reimbursement, status: 'approved', problem_type: nil,
