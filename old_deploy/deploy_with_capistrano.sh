...
usage() {
  ...
  echo "  logs        Check Docker container logs"
  ...
  echo "  $0 restart             # Restart Docker container in staging environment"
...
case "$COMMAND" in
  ...
  setup)
    ...
    bundle exec cap $ENVIRONMENT utils:setup
    ;;
  rollback)
  ...
  status)
    ...
    bundle exec cap $ENVIRONMENT docker:status
    bundle exec cap $ENVIRONMENT utils:check_application
    ;;
  logs)
    echo "=== Checking logs in ${ENVIRONMENT} environment ==="
    bundle exec cap $ENVIRONMENT docker:logs
    ;;
  restart)
    echo "=== Restarting Docker container in ${ENVIRONMENT} environment ==="
    bundle exec cap $ENVIRONMENT docker:restart
    ;;
  diagnose)
    echo "=== Running diagnostics in ${ENVIRONMENT} environment ==="
    # Check SSH connection
    echo "Step 1: Checking SSH connection..."
    bundle exec cap $ENVIRONMENT utils:check_server
  ...
  assets)
    ...
    # Source the common configuration if available
    if [ -f "$(dirname "$0")/config/deploy_config.sh" ]; then
      source "$(dirname "$0")/config/deploy_config.sh"
      
      # Run the asset precompilation
      ...
      echo "Error: deploy_config.sh not found. Running standard asset precompilation."
      bundle exec cap $ENVIRONMENT deploy:assets:precompile
      bundle exec cap $ENVIRONMENT docker:restart
    fi
    ;;
