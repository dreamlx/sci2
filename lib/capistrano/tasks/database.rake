  ...
  task :test_connection do
    on roles(:db) do
      within release_path do
        ...
        execute :rvm, fetch(:rvm_ruby_version), :do, :bundle, :exec, :rails, :runner, "\"#{runner_command}\"",
...
before 'deploy:migrate', 'database:create_mysql_setup' if fetch(:database_config, '').include?('mysql2')

# Setup development database symlink when using development environment
before 'deploy:migrate', 'database:setup_development_symlink' if fetch(:rails_env) == 'development'
