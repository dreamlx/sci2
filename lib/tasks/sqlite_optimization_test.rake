namespace :sqlite do
  ...
  task test_optimization: :environment do
    ...
    # 测试不同优化级别
    levels = %i[safe moderate aggressive]
    test_results = {}
    ...
    if test_results.values.all? { |r| r[:success] }
      ...
      test_results.each do |level, result|
        ...
        puts "#{level.to_s.upcase}: #{'+' if improvement > 0}#{improvement}% 性能变化"
      end
    end
  ...
  task benchmark_import: :environment do
    ...
    results.each do |level, result|
      ...
      puts "#{level.to_s.upcase}: #{result[:records_per_second]} 记录/秒 (#{'+' if improvement > 0}#{improvement}%)"
    end
  end
