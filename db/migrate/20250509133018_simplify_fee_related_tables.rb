class SimplifyFeeRelatedTables < ActiveRecord::Migration[7.1]
  ...
  def down
    ...
    # Add indexes as they were in the original schema
    add_index :fee_detail_selections, [:fee_detail_id, :work_order_id, :work_order_type], name: "index_fee_detail_selections_on_fee_detail_and_work_order", unique: true
    # index for fee_detail_id is covered by references :fee_detail
    # index for verifier_id is covered by references :verifier
    add_index :fee_detail_selections, [:work_order_type, :work_order_id], name: "index_fee_detail_selections_on_work_order"
