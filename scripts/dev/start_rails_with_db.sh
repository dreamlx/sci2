...
echo "🔍 检查数据库连接..."
for i in {1..30}; do
    if $DOCKER_COMPOSE exec -T db pg_isready -U sci2 -d sci2_development > /dev/null 2>&1; then
        echo "✅ 数据库已就绪"
        break
    fi
    echo "⏳ 等待数据库启动... ($i/30)"
...
# 运行数据库迁移
echo "🗃️  运行数据库迁移..."
$DOCKER_COMPOSE exec -T app bundle exec rails db:create db:migrate db:seed

# 创建管理员用户（如果不存在）
echo "👤 创建管理员用户..."
$DOCKER_COMPOSE exec -T app bundle exec rails runner "
