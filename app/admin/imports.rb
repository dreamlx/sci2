  ...
  page_action :import_operation_histories, method: :post do
    ...
    if result[:success]
      ...
      if result[:unmatched].to_i > 0
        ...
        notice_message += if unmatched_doc_numbers.size <= 5
                            " #{result[:unmatched]} 未匹配: #{unmatched_doc_numbers.join(', ')}."
                          else
                            " #{result[:unmatched]} 未匹配: #{unmatched_doc_numbers.first(5).join(', ')} 等."
                          end
      end
      ...
      alert_message = "导入失败: #{if result[:error_details]
  ...
  page_action :import_problem_codes, method: :post do
    ...
    begin
      ...
      if result[:success]
        ...
        if result[:updated_fee_types].to_i > 0 || result[:updated_problem_types].to_i > 0
          notice_message += " #{result[:updated_fee_types]} 费用类型更新, #{result[:updated_problem_types]} 问题类型更新."
        end
