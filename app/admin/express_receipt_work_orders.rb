ActiveAdmin.register ExpressReceiptWorkOrder do
  ...
  controller do
    def scoped_collection
      super.includes(:reimbursement, :creator, reimbursement: %i[current_assignee active_assignment])
    end

    def create
      super do |_format, resource|
  ...
  filter :creator
  filter :created_at
  filter :reimbursement_current_assignee_id, as: :select, collection: lambda {
  ...
  collection_action :import, method: :post do
    ...
    if result[:success]
      ...
      # 增强的错误消息，提供更清晰的错误信息
      error_msg = if result[:error_details]
  ...
  collection_action :export_csv, method: :get do
    work_orders = ExpressReceiptWorkOrder.includes(
      :creator,
      reimbursement: %i[current_assignee active_assignment]
      ...
      tab '状态变更历史' do
        panel '状态变更历史' do
          table_for resource.work_order_status_changes.order(changed_at: :desc) do
            ...
            column :to_status
            column :changed_at
            column :changer do |change|
              change.changer&.email
            end
          end
        end
      end
  ...
  form do |f|
    f.inputs '快递收单工单信息' do
      f.input :reimbursement_id, as: :select,
                                 collection: Reimbursement.all.map { |r|
                                   ["#{r.invoice_number} - #{r.applicant}", r.id]
                                 },
