ActiveAdmin.register AuditWorkOrder do
  ...
  controller do
    ...
    def create
      ...
      if _audit_work_order_params[:submitted_fee_detail_ids].present?
        ...
        @audit_work_order.instance_variable_set(:@_direct_submitted_fee_ids,
                                                _audit_work_order_params[:submitted_fee_detail_ids])
    ...
    def update
      ...
      if update_params[:submitted_fee_detail_ids].present?
        ...
        if @audit_work_order.respond_to?(:submitted_fee_detail_ids=)
          @audit_work_order.submitted_fee_detail_ids = update_params[:submitted_fee_detail_ids]
        end
      end
  ...
  filter :creator, as: :select, collection: lambda {
    begin
      collection = AdminUser.accessible_by(current_ability).map do |u|
  ...
  member_action :approve, method: :get do
    if resource.approved? || resource.rejected?
      redirect_to admin_audit_work_order_path(resource),
                  alert: '工单已审核或已拒绝，无法再次操作。'
    end
  ...
  index do
    ...
    column '费用明细', :fee_details do |wo|
      if wo.fee_details.any?
        wo.fee_details.map do |fd|
          link_to("##{fd.id}",
                  admin_fee_detail_path(fd)) + " #{fd.fee_type} #{number_to_currency(fd.amount, unit: '¥')}"
    ...
    column '操作' do |work_order|
      ...
      if work_order.editable? # Using new editable? logic from model
        ...
        links << link_to('删除', admin_audit_work_order_path(work_order), method: :delete, data: { confirm: '确定要删除吗?' },
  ...
  collection_action :export_csv, method: :get do
    ...
    csv_data = CSV.generate(headers: true, force_quotes: true) do |csv|
      csv << %w[ID 报销单号 处理意见 审核结果 问题类型 创建人 创建时间 费用明细单号]

      work_orders.find_each do |wo|
        problem_types = if wo.problem_types.any?
                          wo.problem_types.map { |pt| "#{pt.legacy_problem_code}-#{pt.title}" }.join(', ')
                        else
