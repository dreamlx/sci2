ActiveAdmin.register FeeDetail do
  ...
  filter :fee_type
  filter :verification_status, as: :select, collection: %w[pending problematic verified]
  filter :fee_date
  ...
  collection_action :new_import, method: :get do
    render 'admin/shared/import_form_with_progress', locals: {
      ...
      instructions: [
        '请上传CSV或Excel格式文件',
        '文件必须包含以下列：报销单单号,费用id,费用类型,原始金额,费用发生日期',
      ...
      tab '基本信息' do
        attributes_table do
          ...
          row :reimbursement do |fee_detail|
            if fee_detail.reimbursement
              link_to fee_detail.document_number,
                      admin_reimbursement_path(fee_detail.reimbursement)
            end
          end
          row :external_fee_id
          row :fee_type
        ...
        panel '附件列表' do
          if resource.attachments.attached?
            div class: 'attachments-grid',
                style: 'display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin: 20px 0;' do
  ...
  form do |f|
    f.inputs '费用明细信息' do
      ...
      f.input :verification_status, as: :select, collection: %w[pending problematic verified],
                                    include_blank: false
