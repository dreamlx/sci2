...
class OptimizedReimbursementImportService
  ...
  def import(test_spreadsheet = nil)
    ...
    begin
      ...
      # 3. 批量处理（禁用回调以提升性能）
      disabled_callbacks = [
        %i[save after update_status_based_on_fee_details],
        %i[create after update_notification_status],
        %i[update after update_notification_status]
      ]
  ...
  def batch_update_statuses(validated_data)
    ...
    reimbursements_to_update.each do |reimbursement|
      external_status = validated_data.find do |d|
        d[:invoice_number] == reimbursement.invoice_number
      end&.dig(:external_status)

      next unless external_status.present?

      # 使用模型方法确定内部状态
      new_status = reimbursement.determine_internal_status_from_external(external_status)

      # 只有状态发生变化或外部状态更新时才保存
      next unless new_status != reimbursement.status || external_status != reimbursement.last_external_status
  ...
  def parse_all_rows(test_spreadsheet)
    ...
    raise ArgumentError, "缺少必要的列: #{missing_headers.join(', ')}" unless missing_headers.empty?
  ...
  def process_in_batches(data_array, &)
    @batch_manager.send(:process_in_batches, data_array, &)
  end
