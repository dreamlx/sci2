...
class ProblemCodeMigrationService
  ...
  def self.migrate_fee_types
    ...
    FeeType.find_or_create_by(code: '00', title: '个人费用', meeting_type: '个人')
    FeeType.find_or_create_by(code: '01', title: '学术费用', meeting_type: '学术论坛')
  ...
  def self.generate_unique_code(_fee_type, index)
    # Generate a unique code for fee_type
    base_code = (index + 10).to_s # Start from "10" to avoid conflicts

    # Ensure code is unique
    base_code = (base_code.to_i + 1).to_s while FeeType.exists?(code: base_code)

    base_code
  end

  def self.generate_unique_problem_code(_problem_type, fee_type, index)
    # Generate a unique code for problem_type within fee_type
    base_code = (index + 10).to_s # Start from "10" to avoid conflicts

    # Ensure code is unique within fee_type
    base_code = (base_code.to_i + 1).to_s while ProblemType.exists?(code: base_code, fee_type_id: fee_type.id)
  ...
  def self.get_sop_description(problem_type)
    ...
    if problem_type.respond_to?(:problem_descriptions)
      description = problem_type.problem_descriptions.first&.description
    end
    description.presence || '标准操作流程待定' # Default if not available
  end

  def self.get_standard_handling(_problem_type)
