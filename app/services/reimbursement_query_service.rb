class ReimbursementQueryService
  ...
  def apply_filters(query, params)
    ...
    # 发票号过滤
    query = query.where('invoice_number LIKE ?', "%#{params[:invoice_number]}%") if params[:invoice_number].present?

    # 申请人过滤
    query = query.where('applicant LIKE ?', "%#{params[:applicant]}%") if params[:applicant].present?
