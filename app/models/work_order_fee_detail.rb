...
class WorkOrderFeeDetail < ApplicationRecord
  ...
  def self.ransackable_attributes(_auth_object = nil)
    %w[created_at fee_detail_id id id_value updated_at work_order_id]
  end
  ...
  after_commit :update_fee_detail_status, on: %i[create update]
  ...
  def update_fee_detail_status
    ...
    # 更新报销单状态
    # 确保报销单状态与费用明细状态保持一致
    return unless fee_detail&.reimbursement&.persisted?
