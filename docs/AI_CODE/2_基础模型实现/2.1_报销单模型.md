...
RSpec.describe Reimbursement, type: :model do
  ...
  describe "associations" do
    ...
    it { should have_many(:fee_details).dependent(:destroy) }
    it { should have_many(:operation_histories).dependent(:destroy) }
  end
  ...
  describe "#all_fee_details_verified?" do
    ...
    context "when some fee details are not verified" do
      before do
        create_list(:fee_detail, 2, document_number: reimbursement.invoice_number, verification_status: 'verified')
...
2. 在 Rails 控制台中手动测试模型（仅供人工验证，不要在自动化流程中执行）：
   ...
   # 测试关联关系
   reimbursement.work_orders.create(type: "AuditWorkOrder", status: "pending")
   reimbursement.audit_work_orders.count # 应返回 1
   ```
