...
2. **自动分配**：根据预设规则自动分配新导入的报销单
   - **导入时自动分配**：报销单导入时，如果当前审批节点为"审核"且当前审批人匹配公司审核员，则自动分配
   - **操作历史自动分配**：操作历史导入时，如果报销单未分配且操作类型为"加签"且操作人匹配审核员，则自动分配
...
自动分配功能通过修改导入服务实现，在报销单和操作历史导入时自动识别审核员并分配：

```ruby
# app/models/admin_user.rb - 新增辅助方法
def self.find_by_name_substring(name_substring)
  return nil unless name_substring.present?
  where("name LIKE ?", "%#{name_substring}%").first
end

# app/services/reimbursement_import_service.rb - 导入时自动分配
...
# app/services/operation_history_import_service.rb - 操作历史自动分配
